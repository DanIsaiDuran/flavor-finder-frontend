        ...
        <nav className='bg-primary-700 text-black mt-4 mb-8 mx-6 md:mx-12 py-3 rounded-full'>
            <div className='flex flex-wrap items-center justify-between mx-auto px-4'>
                ...
                <div className='hidden sm:flex space-x-8 font-bold'>
                    ...
                    <NavLink to="*" className={({ isActive }) => (isActive ? 'underline decoration-2 hover:text-white' : 'hover:text-white')}>Nosotros</NavLink>

                </div>
                <div className='hidden sm:flex rounded-full flex-grow-0 font-bold bg-primary-900 px-3 py-2'>
                    {user ? (
                        <button className='hover:text-white' onClick={logout}>Cerrar sesión</button>
                    ) :
                    (
                        <button className='hover:text-white' onClick={handleNavigate}>Iniciar sesión</button>
                    )}
