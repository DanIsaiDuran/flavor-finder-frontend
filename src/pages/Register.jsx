...
const Register = () => {
  ...
  const [credentials, setCredentials] = useState({username: "", email: "", password: "", roleRequest: {
    roleListName: ["USER"]
  }});
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      if (credentials.password !== confirmPassword && confirmPassword) {
        setErrors({...errors, "confirmPassword": "Las constraseñas deben coincidir"});
      }
      else {
        delete errors.confirmPassword;
        setErrors({...errors});
      }
    }, 500); // Delay validation
    ...
    <>
      <div className="bg-primary-400 h-screen overflow-hidden flex items-center justify-center">
            <div className="bg-cardColor-900 lg:w-6/12 md:7/12 w-8/12 shadow-3xl rounded-xl shadow-md shadow-gray-700">
                ...
                <div className="flex flex-col text-lg mb-6 md:mb-8">
                    <div className='relative w-full'>
                      <FontAwesomeIcon icon={faAt} className="absolute left-3 top-1/2 transform -translate-y-1/2" width="24" />
                      <input type="text" id="email" name="email" value={credentials.email || ""} className="bg-gray-200 rounded pl-12 py-2 md:py-4 focus:outline-none w-full" placeholder="Correo electronico" onChange={handleChange}/>
                    </div>
                    {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email}</p>}
                </div>
                <div className="flex flex-col text-lg mb-6 md:mb-8">
                  <div className='relative w-full'>
                  <svg className="absolute left-3 top-1/2 transform -translate-y-1/2" width="24" viewBox="0 0 24 24">
                ...
                <div className="flex flex-col text-lg mb-6 md:mb-8">
                  <div className='relative w-full'>
                    <svg className="absolute left-3 top-1/2 transform -translate-y-1/2" viewBox="0 0 24 24" width="24">
                ...
                <div className="flex flex-col text-lg mb-6 md:mb-8 w-full">
                  <div className="relative w-full">
                    <svg className="absolute left-3 top-1/2 transform -translate-y-1/2" viewBox="0 0 24 24" width="24">
                      <path d="m18.75 9h-.75v-3c0-3.309-2.691-6-6-6s-6 2.691-6 6v3h-.75c-1.24 0-2.25 1.009-2.25 2.25v10.5c0 1.241 1.01 2.25 2.25 2.25h13.5c1.24 0 2.25-1.009 2.25-2.25v-10.5c0-1.241-1.01-2.25-2.25-2.25zm-10.75-3c0-2.206 1.794-4 4-4s4 1.794 4 4v3h-8zm5 10.722v2.278c0 .552-.447 1-1 1s-1-.448-1-1v-2.278c-.595-.347-1-.985-1-1.722 0-1.103.897-2 2-2s2 .897 2 2c0 .737-.405 1.375-1 1.722z"/>
                    </svg>  
                ...
                <button type='submit' className="bg-gradient-to-b from-secondary to-secondary font-medium p-2 md:p-4 text-white uppercase w-full rounded">Registrarse</button>
                </form>
